...
chomp($geneSymbolCol);
$geneSymbolCol--;
my $expFile="GSE48424_expression_data.txt";
my $gplFile="GPL6480-9577.txt";
my $expFileWF="PreeclampsiaMatrix.txt";
my %hash=();
my @sampleName=();
...
while(my $gpl=<GPL>)
{
	...
	chomp($gpl);
	my @gplArr=split(/\t/,$gpl);
	if((exists $gplArr[$geneSymbolCol]) && ($gplArr[$geneSymbolCol] ne '') && ($gplArr[$geneSymbolCol] ne '---') && ($gplArr[$geneSymbolCol] !~ /.+\s+.+/))
	{
...
######课程链接2: https://www.biowolf.cn/shop/
######课程链接3: https://www.biowolf.cn/shop/mobile/index.php
